    ...
    for file_path in directory.rglob('*.bin'):
        with open(file_path, 'rb') as f:
            ...
            while True:
                ...
                print(f"Packet {packet_index}: {timestamp} | {src_mac} | {dst_mac} | {rssi} | {channel} | {payload_len}")
                measurement = Measurement(device_id=device.mac_address, happened_at=datetime.fromtimestamp(timestamp), additional_data={
                    'source_mac': src_mac,
